...
def detect_statement_type(pdf_path: str) -> str:
...
def detect_and_parse(pdf_path: str) -> tuple[str, list[ExpenseCreate]]:
    """Auto-detect statement type and parse it. Returns (type, transactions)."""
    file_type = detect_statement_type(pdf_path)

    if file_type == "credit_card":
        transactions = parse_credit_card_statement(pdf_path)
    elif file_type == "upi":
        transactions = parse_upi_statement(pdf_path)
    else:
